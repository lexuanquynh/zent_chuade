...
#define TEN_FILE "students.data"
#define TEXT_FILE "ketqua.txt"
...
void hienThiHocSinh(Hocsinh hocsinh)
{
...
//function hien thi menu ra man hinh
void hienthiHocSinh()
{
...
int soLuongHocBong()
{
	...
	if (input_file.is_open())
	{
		...
		while (!input_file.eof() && output_file.is_open())
		{
			...
			if (hocsinh.diemTB > 8.5 && (hocsinh.hanhkiem == KHA || hocsinh.hanhkiem == TOT))
			{
				soluongHB++;
		...
		if (soluongHB == 0)
			cout << "Khong co hoc sinh nao dat" << endl;
...
void Menu()
{
	...
	while (1)
	{
		...
		cout << "5. Sap xep danh sach theo chieu tang" << endl;
